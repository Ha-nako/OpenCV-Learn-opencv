...
int main() {
	...
	for (size_t i = 0; i < circles.size(); i++)
	{
		...
		if (Lei == 4)
		{
			putText(img, "4", Point(circles[i][0] + 1, circles[i][1] + 5), FONT_HERSHEY_DUPLEX, 1, Scalar(0, 69, 255), 1);
			//	文字：	  ↑内容					       ↑字体（随机）		↑比例		      厚度↑
		}
		if (Lei == 0)
		{
			putText(img, "0", Point(circles[i][0] + 1, circles[i][1] + 5), FONT_HERSHEY_DUPLEX, 1, Scalar(0, 69, 255), 1);
			//	文字：	  ↑内容					       ↑字体（随机）		↑比例		      厚度↑
		}
		if (Lei == 2)
		{
